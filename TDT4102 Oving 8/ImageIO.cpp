...
int SaveImage(const Image &image, const std::string &filename) {
    ...
    writeUnsignedInt(file, image.GetWidth());
    writeUnsignedInt(file, image.GetHeight());
    ...
    // http://en.wikipedia.org/wiki/BMP_file_format#Pixel_storage
    int imagePitch = image.GetWidth() * BYTES_PER_PIXEL;
    int padBytes = rowSize(image.GetWidth(), BITS_PER_PIXEL) - (image.GetWidth() * BYTES_PER_PIXEL);
    ...
    for (int y = 0; y < image.GetHeight(); y++) {
        ...
        for (unsigned int x = 0; x < image.GetWidth(); x++) {
            ...
            writeChar(file, pixel.blue);
            writeChar(file, pixel.green);
            writeChar(file, pixel.red);
        }
